...
sp=True       #spin polarized
orbitals=False #orbital projected

rom=6000 #omega points
...
#kpath generation for band plot
...
#updating ksum.input for band plotting
...
#updating ksum.input for DOS
os.popen("cp ksum.input ./imp.0/plot/")
fi=open('./imp.0/plot/ksum.input','r')
datax=fi.readlines()
fi.close()
datax[1]=str(rom)+"\n"
...
#running dmft_ksum_dos
print('Running dmft_ksum_dos...')
cmd = "cd ./imp.0/plot &&"+" "+para_com+" "+"dmft_ksum_dos"
out, err = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
print('Complete.\n')
