...
import sys
sys.path.insert(0, "/home/uthpala/Documents/Research/projects/DMFT/codes/DFTDMFT/bin/")
...
## We don't need this anymore for siesta
...
if __name__ == '__main__':
 ...
 ############ Initial Preparation ########################
   ...
   for itt in range(p['Niter']):
    ...
    #We don't need to create INCAR for siesta 
#      CreateINCAR(pV)
...
#         f.close()
      ...
      #rename wannier90.win file to systemname.win to match with siesta input
     # os.popen('mv wannier90.win SrVO3.win')
      #do stuff here to create a complete wannier90.win file
      
      #running siesta+wannier90
      #creating the .nnkp file required for the siesta+wannier90 run 
      cmd = "wannier90.x -pp SrVO3" #p['path_bin']+"wannier90.x SrVO3"
      out, err = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
      print out , err
      
      #running siesta
      cmd = 'siesta <SrVO3.fdf>SrVO3.out'
      out, err = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
      print out,err
      
      
      #We don't need vasp anymore
...
#      print os.popen("cp CHGCAR CHGCAR."+str(itt)).read()
      ...
      #We need to extract fermi from SrVO3.out at this point.Initially we used a guess in DFT_mu.out. Updates this file.   
      #DFT.Read_NBANDS()
      #DFT.Read_EFERMI()
...
#         print os.popen("rm wannier90.eig").read()
      ...
      #renaming .eigW to .eig for wannier90 to read
      os.popen("mv SrVO3.eigW SrVO3.eig")

      #if itt<p['Niter']-1:
      main_out.write( '-------------- Running wannier 90 '+str(itt+1)+'----------------' )
      main_out.write('\n')
      main_out.flush()
      cmd = "wannier90.x SrVO3" #to get .chk 
      out, err = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
      print out , err
      
      #renaming SrVO3.chk to wannier90.chk
      os.popen("mv SrVO3.chk wannier90.chk")
      
      #renaming SrVO3.eig to wannier90.eig
      os.popen("mv SrVO3.eig wannier90.eig")
      ...
      for it in range(p['Nit']):
         ...
         #read energy from siesta output     
         #fis = open('siesta_ETOT')
         #DFT.E = float(fis.readlines()[0])
         
         #read VASP.log instead of VASP.output 
         if itt==0:
            ETOT=DFT.E+E_POT # Total Ground state energy + E POT?
            ETOT2=DFT.E+E_POT
         else:
            ...
            #Added by Vijay
            cmd = 'grep "Band Struct." SrVO3.out > Band_print'
            os.system(cmd)
            SBE=open('Band_print')
            for line in SBE:
            	package = line.split()[4]
            f1=open('./BandStructure', 'w+')
            print >> f1, package
            f1.close()
            fiEKIN=open('BandStructure')
            EB=float(fiEKIN.readlines()[0])
            #ECOR=float(lines[-1].split()[1])
            fiEKIN.close()
            #ETOT=DFT.E-TrDeltaG+ETOT_imp+VdcNd+VdcNd2-DMFT.EDC_imp
            #ETOT=DFT.E-EB+E_KIN+E_POT+VdcNd3
            #ETOT2=DFT.E-EB+E_KIN-TrDeltaG+ETOT_imp+VdcNd+VdcNd2#-DMFT.EDC_imp
            ETOT=DFT.E-EB+E_KIN+E_POT+VdcNd3-DMFT.EDC
            ETOT2=DFT.E-EB+E_KIN+E_POT2-DMFT.EDC_imp           
